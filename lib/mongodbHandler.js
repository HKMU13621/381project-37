...
async function updateDocument(collection, query, newDoc, method = '$set', oneOrMany = 'one') {
    try {
        let result;

        if (oneOrMany === 'one') {
            result = await executeQuery(() => collection.updateOne(query, { [method]: newDoc }));
        } else if (oneOrMany === 'many') {
            result = await executeQuery(() => collection.updateMany(query, { [method]: newDoc }));
        } else {
